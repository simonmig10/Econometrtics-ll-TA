...
$$\sum^n_{n=1}a x^n=x + x^2 + x^3 + x^4 +...+x^{n-1}$$

- Hvor $a=1$

### Eksempel 2:

$$\sum^n_{n=0} a x^{-1/2n} = x + \sqrt{x} + 1 + \frac{1}{\sqrt{x}} + \frac{1}{x} ... $$
...
Først lad os se hvad dde har tilfældes:
...
$$\sum^t_{k=1}a^{t-k}b$$ 
Hvilket er en geometrtisk serie! Som vi lige har kigget på!
...
Vi kan se hvis $x_0 = \frac{b}{1-a}$ får vi $x_t= \frac{b}{1-a}$ hvilket er illustreret under:
...
Hvis $x_s$ på noget tidspunkt rammer $\frac{b}{1-a}$ vil vi aldrig komme væk fra dette punkt:
